...
public class StockpileConnector extends Connector {
    ...
    public void handleConnection(Location target) {
        ...
        if (shop.getStockpileSet().contains(center)) {
            ...
            CoreProtectLogger.logStockpileRemove(player, shop.getLocation(), center);
        } else {
            if (NormalShops.getInstance().getShopManager().getStockpileOwner(target) != null) {
            ...
            CoreProtectLogger.logStockpileAdd(player, shop.getLocation(), center);
        }
    }
